...
class PDF extends PDFTable{
    ...
    <table border=0 cellpadding=1>
      <tr>
				<td width=290 size=8 style="bold" align="center">  MOVIMIENTO DE INGRESOS Y SALIDAS DE ALMACENES DEL '.$_GET['fini'].' AL '.$_GET['ffin'].' </td>
			</tr>
		</table>';
    $this->SetXY(15,14);
...
//	$pdf->SetFont('Arial','',7,true);
	...
	foreach($p as $pro => $v){					
			...
			$pdf->Cell(85,5,"Almacen: ".$auxalm."-".balm($auxalm),0,0,'L',false);			 
			$pdf->Cell(105,5,"Producto: ".trim($pro)."-". bprod($pro),0,0,'L',false);
			$pdf->Cell(95,5,"Unidad de medida: ".bunimedprod($pro),0,1,'L',false);
			$pdf->Cell(20,5,'DCTO.-NRO',1,0,'L');
			$pdf->Cell(15,5,'FECHA',1,0,'L');
			$pdf->Cell(75,5,'REPARTICION',1,0,'L');
			$pdf->Cell(25,5,'CANT.INGRESOS',1,0,'R');
			$pdf->Cell(25,5,'CANT.SALIDAS',1,0,'R');
			$pdf->Cell(20,5,'DCTOM-NROM',1,0,'R');
			$pdf->Cell(25,5,'CANT.SALDOS',1,0,'R');
			$pdf->Cell(25,5,'PREC.UNITARIO',1,0,'R');
			...
			$pdf->Cell(110,5,'Saldo anterior al '.$_GET['fini'],0,0,'R');
			$pdf->Cell(20,5,number_format($asaldo[0],2),0,0,'R');
			...
			foreach($v as $fila => $row1){
				...
				$pdf->Cell(20,5,$dctonro,0,0,'L');
				$pdf->Cell(15,5,$row1['fecha'],0,0,'L');
				$pdf->Cell(75,5,brepar($row1['codrep']),0,0,'L');
				$pdf->Cell(25,5,number_format($row1['dcant'],2),0,0,'R');
				$pdf->Cell(25,5,number_format($row1['hcant'],2),0,0,'R');
				$pdf->Cell(20,5,$dctonrom,0,0,'R');
				$pdf->Cell(25,5,number_format($scant,2),0,0,'R');
				$pdf->Cell(25,5,number_format($row1['pu'],4),0,0,'R');
			...
			$pdf->Ln(1);
			$pdf->SetFont('Arial','B',7,true);	
			$pdf->Cell(20,5,'Totales',0,0,'L');
			$pdf->Cell(15,5,'',0,0,'L');
			$pdf->Cell(75,5,'',0,0,'L');
			$pdf->Cell(25,5,number_format($toting,2),0,0,'R');
			$pdf->Cell(25,5,number_format($totsal,2),0,0,'R');
			$pdf->Cell(20,5,'',0,0,'R');
			$pdf->Cell(25,5,'',0,0,'R');
			$pdf->Cell(25,5,'',0,0,'R');
