...
class PDF extends PDFTable{
    ...
    <table border=0 cellpadding=1>
      <tr>
				<td width=305 size=10 style="bold" align="center">ESTADO DE PREVENTIVOS Y SUS DESCARGOS SIGEP<br>CARGOS DE CUENTA CON DESCARGOS</td>
			</tr>
		</table>';
    $this->SetXY(15,10);
    $this->htmltable($titulo,1);
		...
		<table border=1>
			<tr>
				...
				<td width=15 style="bold" size=5>FECHA DSCGO.</td>
...
$sql = "SELECT * FROM (";
$sql.= "select m.scta, s.des, k.fecha, m.glosa[1,70] glosa, m.gestion, m.nro,m.idprev, ";
$sql.= "m.usr_upd, m.tipo, c.estado, revisor,sum(monto) monto, sum(monto) descargo ";
$sql.= "from maesigep m ";
...
$sql.= "inner join descsigep c on c.gestion = m.gestion and c.nro = m.nro and c.idprev = m.idprev and c.tipo = m.tipo ";
$sql.= "inner join (SELECT gestion,nro, idprev, tipo, max(id) id,CAST(max(fecha) AS DATE) fecha FROM kardesigep group by 1,2,3,4) k ";
$sql.= "on k.gestion = m.gestion and k.nro = m.nro and k.idprev = m.idprev and k.tipo = m.tipo ";
$sql.= "where estprev='C' AND (c.estado<>'X') ".$cripart;
...
$pdf->Output('sigep_descargados.pdf','I');
unset($pdf);
?>