...
class PDF extends PDFTable{
    ...
    <table border=0 cellpadding=1>
      <tr>
				<td width=305 size=10 style="bold" align="center">ESTADO DE PREVENTIVOS Y SUS DESCARGOS SIGEP<br>SIN AGRUPAR</td>
			</tr>
		</table>';
    $this->SetXY(15,10);
    $this->htmltable($titulo,1);
        ...
        <td width=55 style="bold" size=5>CUENTADANTE</td>
				<td width=60 style="bold" size=5>GLOSA</td>
...
$sql = "select m.fecha, m.glosa[1,50] glosa, m.gestion, m.nro,m.idprev, m.scta, m.usr_upd,m.tipo,s.des[1,42] des, ";
...
$qry = "SELECT * FROM (".$sql.") ".$criterio." ORDER BY des";
$p = $db->prepare($qry);
$p->execute();
...
while($csig = $p->fetch(PDO::FETCH_ASSOC)){
  $c++;
  ...
  $pdf->Cell(60,3,$csig['glosa'],0,0,'L');
