...
class PDF extends PDFTable{
    ...
    <table border=0 cellpadding=1>
      <tr>
				<td width=305 size=10 style="bold" align="center">ESTADO DE PREVENTIVOS Y SUS DESCARGOS SIGEP<br>INCLUYE NRO DE NOTIFICACION</td>
			</tr>
		</table>';
    $this->SetXY(15,10);
    $this->htmltable($titulo,1);
        ...
        <td width=12 style="bold" align="right" size=5>NOTIF.</td>
			</tr>
		</table>';
		$this->SetXY(15,25);
...
while($csig = $p->fetch(PDO::FETCH_ASSOC)){
  ...
  $nrodoc = bnotisig($csig['gestion'],$csig['nro'],$csig['idprev'],$csig['tipo']);
  $pdf->SetFont('Arial','',5,true);
  ...
  $pdf->Cell(15,3,$ndias,0,0,'R');
  $pdf->Cell(12,3,$nrodoc,0,1,'R');
